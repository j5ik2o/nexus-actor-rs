...
impl BaseContext for ContextAdapter {
  fn self_ref(&self) -> Box<dyn ActorRef> {
    ...
    let pid = handle.block_on(async {
      self.context.get_self().await
    });
    let actor_system = handle.block_on(async {
  ...
  fn parent_ref(&self) -> Option<Box<dyn ActorRef>> {
    let handle = tokio::runtime::Handle::current();
    let parent_opt = handle.block_on(async {
  ...
  async fn spawn_child(&self, name: &str, _factory: Box<dyn crate::actor::core_types::ActorFactory>) -> Box<dyn ActorRef> {
