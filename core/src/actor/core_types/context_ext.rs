use crate::actor::core::{Props, Actor};
use crate::actor::core_types::{BaseContext, BaseActor, ActorRef, PidActorRef};
    ...
    B: BaseActor + 'static
{
    ...
    Props::from_async_actor_receiver(move |ctx| {
        ...
        async move {
            Actor::handle(&mut migrated, ctx).await
        }
    }).await
}
...
mod tests {
    ...
    use crate::actor::core_types::{Message, BaseActorError};
            ...
            if let Some(adapter) = ctx_ref.as_any().downcast_ref::<ContextAdapter>() {
                ...
                let child_props = Props::from_async_actor_receiver(move |ctx| {
                    ...
                    async move {
                        ...
                        actor.handle(&base_ctx).await
                            .map_err(|e| ActorError::ReceiveError(
                                crate::actor::core::ErrorReason::new(e.to_string(), 0)
                            ))
                    }
                }).await;
                ...
                let _child2 = adapter.spawn_child_actor(&child_name2, move || {
                    TestActor { count: count2.clone() }
                }).await;
            }
    ...
    async fn test_spawn_child_with_props() {
        ...
        let parent_props = Props::from_async_actor_receiver(move |ctx| {
            let mut actor = ParentActor { child_count: child_count.clone() };
