...
pub trait BaseSpawnerExt {
    ...
    async fn spawn_base_actor_named<F, B>(
        &mut self, 
        factory: F, 
        name: &str
...
mod tests {
    ...
    use crate::actor::core_types::{BaseActor, BaseContext, BaseActorError, Message};
    ...
    async fn test_spawn_base_actor() {
        ...
        let pid = root_context.spawn_base_actor(move || TestActor { count: count_clone.clone() }).await;
    ...
    async fn test_spawn_base_actor_named() {
        ...
        let result = root_context.spawn_base_actor_named(
            move || TestActor { count: count_clone.clone() }, 
            "test-actor"
        ).await;
        assert!(result.is_ok());
