...
pub trait Lifecycle: Send + Sync {
    ...
    async fn pre_restart(&mut self, ctx: &dyn Context, reason: Option<Box<dyn std::error::Error>>) -> Result<(), Box<dyn std::error::Error>>;
...
impl<T: Send + Sync> Lifecycle for T {
    ...
    async fn pre_restart(&mut self, _ctx: &dyn Context, _reason: Option<Box<dyn std::error::Error>>) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
