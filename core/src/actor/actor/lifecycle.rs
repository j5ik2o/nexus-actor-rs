...
pub trait Lifecycle: Send + Sync {
    ...
    async fn pre_restart(&mut self, ctx: &Context, reason: Option<Box<dyn std::error::Error>>) -> Result<(), Box<dyn std::error::Error>>;
    ...
    async fn terminated(&mut self, ctx: &Context, who: &crate::actor::pid::Pid) -> Result<(), Box<dyn std::error::Error>>;
}
...
impl<T: Send + Sync> Lifecycle for T {
    ...
    async fn pre_restart(&mut self, _ctx: &Context, _reason: Option<Box<dyn std::error::Error>>) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
    ...
    async fn terminated(&mut self, _ctx: &Context, _who: &crate::actor::pid::Pid) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
}
