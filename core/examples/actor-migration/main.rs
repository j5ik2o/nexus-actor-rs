...
async fn main() {
  ...
  let ping_props = MigrationHelpers::props_from_base_actor_fn(|| PingActor::new("PingActor".to_string())).await;
  let pong_props = MigrationHelpers::props_from_base_actor_fn(|| PongActor::new("PongActor".to_string())).await;
  ...
  println!(
    "\n=== Example completed! Total messages exchanged: {} ===",
    total_messages
  );
}
