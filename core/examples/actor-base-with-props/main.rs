...
use nexus_actor_core_rs::actor::context::{SpawnerPart, SenderPart};
use nexus_actor_core_rs::actor::core::{Props, ActorError};
...
impl nexus_actor_core_rs::actor::core::Actor for ParentActor {
    async fn receive(&mut self, mut context: nexus_actor_core_rs::actor::context::ContextHandle) -> Result<(), ActorError> {
        ...
        if let Some(create_child) = msg.to_typed::<CreateChild>() {
            println!("[Parent] Creating child '{}' with_props={}", create_child.name, create_child.with_props);
            ...
            if create_child.with_props {
                ...
                let child_props = MigrationHelpers::props_from_base_actor_fn(move || {
                    ChildActor { 
                        name: child_name.clone(),
                        counter: counter.clone()
                    }
...
async fn main() {
    ...
    println!("\nTotal child messages processed: {}", child_counter.load(Ordering::SeqCst));
    println!("\n=== Example completed! ===");
}