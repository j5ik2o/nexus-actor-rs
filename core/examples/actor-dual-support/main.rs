...
use nexus_actor_core_rs::actor::context::{BaseSpawnerExt, SpawnerPart, SenderPart, StopperPart};
use nexus_actor_core_rs::actor::core::{Actor, Props, ActorError};
use nexus_actor_core_rs::actor::core_types::{BaseActor, BaseContext, BaseActorError, Message};
...
async fn main() {
    ...
    let modern_pid1 = root_context.spawn_base_actor(|| ModernGreeter { 
        id: "M1".to_string() 
    }).await;
    
    let modern_pid2 = root_context.spawn_base_actor_named(
        || ModernGreeter { id: "M2".to_string() },
        "modern-greeter-2"
    ).await.unwrap();
    ...
    let msg1 = MessageHandle::new(Greeting { name: "Alice".to_string() });
    let msg2 = MessageHandle::new(Greeting { name: "Bob".to_string() });
    let msg3 = MessageHandle::new(Greeting { name: "Charlie".to_string() });
    let msg4 = MessageHandle::new(Greeting { name: "Diana".to_string() });
    ...
    let mixed_msg = MessageHandle::new(Greeting { name: "Everyone".to_string() });
