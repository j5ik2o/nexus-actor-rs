...
## 参考モデルについて
- 本ドキュメントには、`Shared` 抽象や async ランタイム構成をイメージしやすくするためのコード断片（std/tokio 版、RP2040 版、RP2350 版など）を参考として記載している。
- これらは設計上の観点を示すための例であり、実際の実装では最適なデータ構造・安全性・性能要件を踏まえて別途設計し直すこと。
- そのままコピーして適用するのではなく、実プロジェクトの要件に合わせてアーキテクチャを再検討する。
