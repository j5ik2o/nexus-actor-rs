...
use crate::api::{MessageDispatcher, MessageEnvelope};
...
pub(crate) fn create_ask_handles<Resp>() -> (AskFuture<Resp>, MessageDispatcher)
where
  Resp: Element, {
  ...
  let responder = MessageDispatcher::with_drop_hook(dispatch, drop_hook);
  (future, responder)
}
