...
pub struct RcStack<T> {
  inner: SharedStack<RcShared<RefCell<StackBuffer<T>>>, T>,
}
...
impl<T> SharedStackHandle<T> for RcShared<RefCell<StackBuffer<T>>> {
