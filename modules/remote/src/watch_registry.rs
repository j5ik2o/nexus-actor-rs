...
impl WatchRegistry {
  ...
  pub async fn watch(&self, watcher_id: &str, watchee: Pid) -> bool {
  ...
  pub async fn unwatch(&self, watcher_id: &str, watchee: &Pid) -> bool {
  ...
  pub async fn remove_watchee(&self, watcher_id: &str, watchee: Option<&Pid>) -> bool {
    match watchee {
      Some(pid) => self.unwatch(watcher_id, pid).await,
      None => self.prune_if_empty(watcher_id).await,
    }
  }
