...
impl ActorMetrics {
    pub fn new(reader: impl MetricReader) -> Result<Self, opentelemetry::metrics::MetricsError> {
        ...
        let actor_mailbox_length = meter.i64_observable_gauge("nexus_actor_actor_mailbox_length")
        ...
        Ok(ActorMetrics {
            meter: meter.clone(),
            actor_failure_count: meter.u64_counter("nexus_actor_actor_failure_count")
                .with_description("Number of actor failures")
                .with_unit("1")
                .try_init()?,
            actor_mailbox_length,
            actor_message_receive_histogram: meter.f64_histogram("nexus_actor_actor_message_receive_duration_seconds")
                .with_description("Actor's messages received duration in seconds")
                .with_unit("s")
                .try_init()?,
            actor_restarted_count: meter.u64_counter("nexus_actor_actor_restarted_count")
                .with_description("Number of actors restarts")
                .with_unit("1")
                .try_init()?,
            actor_spawn_count: meter.u64_counter("nexus_actor_actor_spawn_count")
                .with_description("Number of actors spawn")
                .with_unit("1")
                .try_init()?,
            actor_stopped_count: meter.u64_counter("nexus_actor_actor_stopped_count")
                .with_description("Number of actors stopped")
                .with_unit("1")
                .try_init()?,
            dead_letter_count: meter.u64_counter("nexus_actor_deadletter_count")
                .with_description("Number of deadletters")
                .with_unit("1")
                .try_init()?,
            futures_started_count: meter.u64_counter("nexus_actor_futures_started_count")
                .with_description("Number of futures started")
                .with_unit("1")
                .try_init()?,
            futures_completed_count: meter.u64_counter("nexus_actor_futures_completed_count")
                .with_description("Number of futures completed")
                .with_unit("1")
                .try_init()?,
            futures_timed_out_count: meter.u64_counter("nexus_actor_futures_timed_out_count")
                .with_description("Number of futures timed out")
                .with_unit("1")
                .try_init()?,
            thread_pool_latency: meter.f64_histogram("nexus_actor_thread_pool_latency_duration_seconds")
...
mod tests {
    ...
    fn test_actor_metrics() {
        ...
        // 検証可能な操作の結果をアサート
        assert_eq!(metrics.get_actor_mailbox_length(), 5, "メールボックスの長さが正しく設定されていません");
