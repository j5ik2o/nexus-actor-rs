  ...
  fn deserialize(&self) -> Result<Arc<dyn RootSerializable>, SerializerError> {
    ...
    for envelope in &self.envelopes {
      ...
      let serializer_raw = u32::try_from(envelope.serializer_id).map_err(|_| {
        SerializerError::DeserializationError("Negative serializer id".to_string())
      })?;

      let serializer_id =
        SerializerId::try_from(serializer_raw).map_err(SerializerError::DeserializationError)?;
